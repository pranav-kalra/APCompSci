public class Main {
    ...
    public static void main(String[] args) {
        ...
        String voters[] = new String[votersList.length];
        ...
        //makes an array of voterId numbers
        int commaPlace = 0;
        for (int a = 0; a < votersList.length; a++) {
            commaPlace = votersList[a].indexOf(',');
            voters[a] = votersList[a].substring(0, commaPlace);
            //or i could have just made it substring(0,7) bc format is always 6 digit voterId
        }
        ...
        for (int b1 = 0; b1 < voters.length; b1++) {
            for (int b2 = b1 + 1; b2 < voters.length; b2++) {
                if (voters[b1].equals(voters[b2])) {
                    votersList[b2] = "000000,0,0,0,0";
                   // System.out.println(b1 + " " + b2);
                    voterFraud = true;
                    voterFrauds++;
                }
            }
            //inside the first loop in case triple voter fraud
            if (voterFraud) {
                votersList[b1] = "000000,0,0,0,0";
                voterFraud = false;
                voterFrauds++;
            }
        }
        ...
        //adds up all of the votes and finds min/max
//        int[] president = new int[5];
...
//        int[] treasurer = new int[5];
        ...
        //highest vote-winner by using three loops and 4 sets of if statements for the four positions but it didn't work
